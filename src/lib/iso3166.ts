/** ISO 3166-1 alpha-3 → alpha-2. Company countryCode is a 2-letter code. */
...
ALPHA2_TO_ALPHA3.XK = "KOS";

export const toIso3166Alpha2 = (code?: string) => {
  const raw = code?.trim().toUpperCase() ?? "";
  if (!raw) return "";
  if (raw.length === 2) return raw;
  return ALPHA3_TO_ALPHA2[raw] ?? raw;
};

export const toIso3166Alpha3 = (code?: string) => {
  const raw = code?.trim().toUpperCase() ?? "";
  if (!raw) return "";
  if (raw.length === 3) return raw;
  return ALPHA2_TO_ALPHA3[raw] ?? raw;
};
