...
import { useListPartners } from "@/api/partners";
import { useListPartnerCommissions } from "@/api/partner-commissions";
import type {
...
const isBusinessType = (value: string | null): value is DomainCommissionsBusinessType =>
  value === "newBusiness" || value === "renewal";

const isEntryType = (value: string | null): value is DomainCommissionsEntryType =>
...
const PartnerCommissionsList = () => {
  ...
  const [partnerId, setPartnerId] = useState(() => searchParams.get("partnerId")?.trim() ?? "");
  ...
  const [businessType, setBusinessType] = useState<DomainCommissionsBusinessType | "ALL">(() => {
    const fromUrl = searchParams.get("businessType");
    return isBusinessType(fromUrl) ? fromUrl : "ALL";
  });
  const [entryType, setEntryType] = useState<DomainCommissionsEntryType | "ALL">(() => {
  ...
  const filters = useMemo(
    ...
    [partnerId, policyId, productId, businessType, entryType, postedFrom, postedTo],
  );
  ...
  const partnerNameById = useMemo(
    () =>
      Object.fromEntries(
        (partnersPage?.items ?? [])
          .filter((p) => p.id)
          .map((p) => [p.id as string, p.name?.trim() || p.id || ""]),
      ),
    [partnersPage?.items],
  );

  const hasFilters =
    Boolean(partnerId.trim()) ||
    ...
    <AppShell>
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-6">
        <div>
          ...
          <h1 className="text-2xl font-semibold tracking-tight">Partner commissions</h1>
          <p className="text-sm text-muted-foreground mt-1">
            Accrued and reversed commission entries posted against policies for partners.
          </p>
        </div>
      </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              ...
              <div className="space-y-1.5">
                ...
                <Select
                  value={businessType}
                  onValueChange={(value) =>
                    setBusinessType(value === "ALL" ? "ALL" : (value as DomainCommissionsBusinessType))
                  }
                >
              ...
              <div className="space-y-1.5">
                ...
                <Select
                  value={entryType}
                  onValueChange={(value) =>
                    setEntryType(value === "ALL" ? "ALL" : (value as DomainCommissionsEntryType))
                  }
                >
                  <SelectTrigger className="h-9">
                    <SelectValue />
              ...
              <div className="space-y-1.5">
                ...
                <DatePicker
                  ...
                  onChange={(d) => setPostedFrom(d ? format(d, "yyyy-MM-dd") : "")}
              ...
              <div className="space-y-1.5">
                ...
                <DatePicker
                  ...
                  onChange={(d) => setPostedTo(d ? format(d, "yyyy-MM-dd") : "")}
                  placeholder="To date"
                  buttonClassName="h-9"
                />
              </div>
            ...
            <Table>
              ...
              <TableBody>
                {isLoading ? (
                  ...
                  items.map((row) => {
                    const partnerName = row.partnerId ? partnerNameById[row.partnerId] : undefined;
                    return (
                      <TableRow
                      ...
                      >
                        ...
                        <TableCell>
                          {row.policyId ? (
                            <Link
                              ...
                              title={row.policyId}
                            >
                              {row.policySerial ?? shortCommissionId(row.policyId)}
                            </Link>
                          ) : (
                        ...
                        <TableCell>
                          {row.productId ? (
                            <Link
                              ...
                              onClick={(e) => e.stopPropagation()}
                            >
                              {row.productName?.trim() || shortCommissionId(row.productId)}
                            </Link>
                          ) : (
                            <span className="text-muted-foreground">—</span>
                          )}
                        </TableCell>
                        <TableCell className="text-sm max-w-[180px] truncate" title={partnerName || row.partnerId}>
                          {row.partnerId ? (
                            <Link
                              to={`/partners/${row.partnerId}`}
                              className="hover:underline"
                              onClick={(e) => e.stopPropagation()}
                            >
                              {partnerName || (
                                <span className="font-mono text-xs">{shortCommissionId(row.partnerId)}</span>
                              )}
                            </Link>
                          ) : (
                            <span className="text-muted-foreground">—</span>
                        ...
                        <TableCell className="text-sm">{businessTypeLabel(row.businessType)}</TableCell>
                        <TableCell>
                          <span
                        ...
                        <TableCell className="text-right font-mono text-xs">
                          <div>{formatCommissionMoney(row.basisAmount, row.currency)}</div>
                          <div className="text-[11px] text-muted-foreground">{basisLabel(row.basis)}</div>
                        </TableCell>
                        ...
                        <TableCell className="text-right">
                          <Button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              if (row.id) navigate(`/partner-commissions/${row.id}`);
                            }}
